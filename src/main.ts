...
interface Clima {
  ...
  precip_por_mes: Record<string, number>;
  dias_chuva: number; dias_sem_chuva: number;
  temp_med: number; temp_max_abs: number; temp_min_abs: number;
  horas_calor_acima_30: number; horas_calor_acima_35: number;
  umid_med: number; horas_umid_abaixo_40: number;
  rad_med: number; vento_med: number; rajada_max: number;
  serie_diaria: Array<{ d: string; tmin: number; tmed: number; tmax: number; prec: number; rad: number; umid: number; wind: number }>;
  temp_med_por_mes: Record<string, number>;
  chuva_extrema_dias: Array<{ d: string; mm: number }>;
}

interface FertOp {
  op: string; n: number; aplicada: number; configurada: number; desvio_pct: number;
  area_ha: number; peso_kg: number; abaixo: number; ok: number; acima: number;
}
interface Fertilizacao {
  inicio: string; fim: string; total_registros: number; area_total_ha: number; peso_total_kg: number;
  operacoes: FertOp[]; total_abaixo: number; total_ok: number; pct_abaixo: number; pct_ok: number;
}

interface Maquinas {
  inicio: string; fim: string;
  alerts: Record<string, number>;
  alerts_por_op: Record<string, number>;
  velocidade_med_kmh: number | null;
  rotacao_med_rpm: number | null;
  vel_por_op: Record<string, number>;
}

interface Pulverizacao {
  inicio: string; fim: string; total_registros: number;
  pressao_zero: number; pressao_media_psi: number; pressao_max_psi: number;
  ops: Record<string, number>;
}

interface Pragas {
  eventos_total: number; insetos_total: number;
  especies: Record<string, { total: number; eventos: number }>;
  serie_diaria: Array<{ d: string; count: number }>;
  nivel_armadilhas: Record<string, string>;
  niveis_contagem: Record<string, number>;
}
...
const D: DataRoot = (window as any).DATA;
...
function el(tag: string, attrs: Record<string, string | number> = {}, parent?: SVGElement) {
...
interface BarDatum { label: string; values: number[]; }
function barChart(container: HTMLElement, labels: string[], series: BarDatum[], opts: {
  yMin?: number; unit?: string; height?: number; stacked?: boolean;
} = {}) {
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    el("text", { x: padL - 8, y: y + 4, "text-anchor": "end", "font-size": 10, fill: "#9ab0a3" },
  ...
  labels.forEach((label, i) => {
    ...
    // x label
    el("text", { x: padL + band * i + band / 2, y: h - 10, "text-anchor": "middle", "font-size": 10.5, fill: "#9ab0a3" },
      svg).textContent = label;

    if (opts.stacked) {
      let acc = 0;
      let total = 0;
      for (const s of series) total += s.values[i] || 0;
      const baseY = padT + plotH;
      series.forEach((s, j) => {
        const v = s.values[i] || 0;
        if (v <= 0) { acc += Math.max(0, v === undefined ? 0 : v); return; }
      ...
      if (series.length === 1) {
        ...
        el("text", { x: x0 + groupW * 0.43, y: baseY - (v / (maxV - minV)) * plotH - 5, "text-anchor": "middle", "font-size": 10, fill: "#e8f0ea" },
...
function axisLegend(container: HTMLElement, items: { label: string; color: string }[]) {
...
function donutChart(container: HTMLElement, items: { label: string; value: number; color: string }[], centerLabel: string) {
  ...
  el("text", { x: cx, y: cy - 2, "text-anchor": "middle", "font-size": 26, "font-weight": 800, fill: "#e8f0ea" },
    svg).textContent = fmt(total);
  el("text", { x: cx, y: cy + 18, "text-anchor": "middle", "font-size": 11, fill: "#9ab0a3" },
    svg).textContent = centerLabel;
  axisLegend(container, items.map(i => ({ label: i.label, color: i.color })));
  container.appendChild(watermarkTotal(total, centerLabel));
}

function watermarkTotal(total: number, label: string) {
...
function areaLineChart(container: HTMLElement, series: Array<{ name: string; color: string; data: Array<{ x: string; y: number }> }>, opts: {
  ...
  const xAt = (d: string) => padL + plotW * (all.indexOf(d) / Math.max(1, all.length - 1));
  const yAt = (v: number) => padT + plotH - ((v - minV) / span) * plotH;

  // x ticks (few)
  const tickEvery = Math.max(1, Math.ceil(all.length / 10));
  all.forEach((d, i) => {
    if (i % tickEvery !== 0 && i !== all.length - 1) return;
    el("text", { x: xAt(d), y: h - 10, "text-anchor": "middle", "font-size": 9.5, fill: "#9ab0a3" },
      svg).textContent = d.slice(5);
  });
...
function mixedBarsArea(container: HTMLElement, labels: string[], bars: { name: string; color: string; data: number[] }, area: { name: string; color: string; data: number[] }, opts: {
  areaLabel?: string; barLabel?: string; height?: number;
} = {}) {
  ...
  for (let i = 0; i <= gridSteps; i++) {
    ...
    const y = padT + plotH - (plotH * i) / gridSteps;
    el("text", { x: w - padR + 6, y: y + 4, "text-anchor": "start", "font-size": 9.5, fill: "#9ab0a3" },
      svg).textContent = fmt(v);
  }

  const xAt = (i: number) => padL + plotW * (i / Math.max(1, labels.length - 1));
  const tickEvery = Math.max(1, Math.ceil(labels.length / 14));

  labels.forEach((lab, i) => {
    if (i % tickEvery === 0 || i === labels.length - 1) {
      el("text", { x: xAt(i), y: h - 10, "text-anchor": "middle", "font-size": 9.5, fill: "#9ab0a3" },
        svg).textContent = lab.slice(5);
    }
...
function gauge(container: HTMLElement, value: number, max: number, label: string) {
  const size = 210, cx = size / 2, cy = size / 2 + 6, r = 82;
  const svg = el("svg", { viewBox: `0 0 ${size} ${size}`, role: "img" });
  container.innerHTML = "";
  container.appendChild(svg);
  // background arc 270deg
  const arc = (start: number, end: number, color: string, width: number, opacity = 1) => {
    const ang = (t: number) => (Math.PI * (180 + t * 270)) / 180;
    el("path", {
  ...
  arc(0, frac, col, 20);
  el("text", { x: cx, y: cy - 8, "text-anchor": "middle", "font-size": 38, "font-weight": 800, fill: col },
    svg).textContent = `${fmt((frac * 100))}%`;
  el("text", { x: cx, y: cy + 18, "text-anchor": "middle", "font-size": 10, fill: "#9ab0a3" },
    svg).textContent = label;
}
...
function renderKpis() {
  ...
  const kpi = (label: string, value: string, sub: string, cls: string) =>
    `<div class="kpi ${cls}"><div class="k-label">${label}</div><div class="k-value">${value}</div><div class="k-sub">${sub}</div></div>`;
...
function renderClima() {
  ...
  barChart(chuvas, meses.map(m => { const [y, mo] = m.split("-"); return `${mo}/${y}`; }),
    [{ label: "Chuva (mm)", values: chuvaV }], { height: 210 });

  const tempMap = C.temp_med_por_mes;
  const tempV = meses.map(m => tempMap[m] || 0);
  barChart(temps, meses.map(m => { const [y, mo] = m.split("-"); return `${mo}/${y}`; }),
    [{ label: "Temp. média (°C)", values: tempV }], { height: 210 });

  mixedBarsArea(diario,
    C.serie_diaria.map(d => d.d),
    { name: "Chuva", color: "#60a5fa", data: C.serie_diaria.map(d => d.prec) },
    { name: "Temp média", color: "#fbbf24", data: C.serie_diaria.map(d => d.tmed) },
    { height: 240, barLabel: "Chuva (mm)", areaLabel: "Temp. média (°C)" });
...
function renderFertilizacao() {
  ...
  barChart(ch,
    topOps.map(o => o.op.length > 22 ? o.op.slice(0, 21) + "…" : o.op),
    [
      { label: "Dose aplicada", values: topOps.map(o => o.aplicada) },
      { label: "Dose configurada", values: topOps.map(o => o.configurada) },
    ], { height: 230 });

  barChart(comp, ["Abaixo da meta", "Dentro da meta", "Acima da meta"],
    [{ label: "Aplicações", values: [F.total_abaixo, F.total_ok, F.operacoes.reduce((a, o) => a + o.acima, 0)] }],
    { height: 210 });
...
function renderPulverizacao() {
  ...
  barChart(ch, ["Média", "Máxima"],
    [{ label: "Pressão (psi)", values: [P.pressao_media_psi, P.pressao_max_psi] }],
    { height: 200, yMin: 0 });

  const opEntries = Object.entries(P.ops);
  barChart(ops, opEntries.map(o => o[0].length > 22 ? o[0].slice(0, 21) + "…" : o[0]),
    [{ label: "Registros", values: opEntries.map(o => o[1]) }],
    { height: 210 });
...
function renderMaquinas() {
  ...
  barChart(ch, alertEntries.map(a => a[0].length > 24 ? a[0].slice(0, 23) + "…" : a[0]),
    [{ label: "Ocorrências", values: alertEntries.map(a => a[1]) }],
    { height: 220 });

  const opEntries = Object.entries(M.alerts_por_op).sort((a, b) => b[1] - a[1]).slice(0, 8);
  barChart(op, opEntries.map(o => o[0].length > 21 ? o[0].slice(0, 20) + "…" : o[0]),
    [{ label: "Alertas", values: opEntries.map(o => o[1]) }],
    { height: 220 });
...
function renderPragas() {
  ...
  donutChart(donut,
    spec.map(([n, v]) => ({ label: n, value: v.total, color: cols[n] || "#a78bfa" }))
      .concat(spec.length === 0 ? [] : [{ label: "Outros", value: 0, color: "#444" }]),
    "insetos");

  areaLineChart(serie,
    [{ name: "Insetos/dia", color: "#f87171", data: G.serie_diaria.map(d => ({ x: d.d, y: d.count })) }],
    { height: 220 });

  const levelOrder = ["DAMAGE", "ALERT", "CONTROL", "LOW"];
  const levelCols: Record<string, string> = { DAMAGE: "#f87171", ALERT: "#fbbf24", CONTROL: "#f59e0b", LOW: "#34d399" };
  const levelVals = levelOrder.map(l => ({ label: l, values: [G.niveis_contagem[l] || 0] }));
  const allLevels = levelVals.filter(v => v.values[0] > 0);
  barChart(nivel, allLevels.map(l => l.label),
    [{ label: "Armadilhas", values: allLevels.map(l => l.values[0]) }],
    { height: 200 });
